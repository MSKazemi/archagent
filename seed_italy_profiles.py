...
from archagent_server import APP_DB, init_app_db, now
    ...
    for profile in PROFILES:
        ...
        if existing:
            con.execute(
                ...
                SET company=?, email=?, countries=?, categories=?, trades=?, min_value=?, max_leads=?, status=?, notes=?, updated_at=?
                WHERE name=?
                """,
                values[1:] + (profile["name"],),
            )
        else:
            con.execute(
                """
                INSERT INTO customer_profiles(name, company, email, countries, categories, trades, min_value, max_leads, status, notes, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                values + (now(),),
            )
