...
def cleanup():
    import archagent_server as s
    con = sqlite3.connect(s.APP_DB)
...
def main():
    import archagent_server as s
    from tender_document_collector import build_tender_dossier, score_italy_fit, select_top_italy_leads
    ...
    workers = s.query_workers({'country': ['Italy'], 'limit': ['1']})['items']
    assert workers, 'expected Italy workers'
    worker_id = workers[0]['id']
    updated = s.verify_worker({'worker_id': worker_id, 'verification_status': 'qualified_test', 'notes': 'production_italy_test verification note', 'capabilities': 'HVAC, retrofit'})
    assert updated['verification_status'] == 'qualified_test', updated
    filtered = s.query_workers({'country': ['Italy'], 'verification_status': ['qualified_test'], 'limit': ['10']})
    assert any(w['id'] == worker_id for w in filtered['items']), filtered
    exported = s.export_workers({'country': ['Italy'], 'verification_status': ['qualified_test'], 'limit': ['10'], 'format': ['csv']})
