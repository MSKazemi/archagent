...
Required env vars:
  ...
  AZURE_OPENAI_DEPLOYMENT — e.g. gpt-4o  (must support vision + JSON mode)
...
def _azure_endpoint() -> tuple[str, str]:
    """Return (url, api_key). Raises EnvironmentError if not configured."""
...
def _call_azure(messages: list[dict], *, json_mode: bool = True) -> tuple[dict, int]:
    """POST to Azure OpenAI. Returns (parsed_response, total_tokens)."""
...
def _extract_json(response: dict) -> dict:
    """Pull JSON from an Azure chat completion response."""
