...
def generate_compliance_matrix(lead: Dict) -> Dict:
    ...
    # Italy-specific procurement context (D.Lgs 36/2023) ----------------------
    is_italy = clean(lead.get('country_label')).lower() == 'italy' or clean(lead.get('performance_country')).upper() == 'ITA' or clean(lead.get('buyer_country')).upper() == 'ITA'
    lead_blob = ' '.join(clean(lead.get(k)) for k in ('title', 'short_title', 'description', 'category', 'source_notice_id'))
    cig = procurement.extract_cig(lead_blob)
    value = lead.get('estimated_value') or lead.get('value_eur')
    inferred_soa = procurement.infer_soa_class(value) if value else None
    pnrr = procurement.detect_pnrr(lead_blob)

    assumption = 'explicit assumption — verify from official tender documents'
    base_items: List = []

    if is_italy:
        # Real Italian document waterfall — named requirements with legal citations.
        cig_evidence = (f'CIG rilevato: {cig}. Verificare su ANAC/FVOE.' if cig
                        else 'Recuperare il CIG dal bando (obbligatorio > €40.000, Art. 19 D.Lgs 36/2023).')
        soa_evidence = (f'Valore € {float(value):,.0f} → classifica minima ≈ {inferred_soa}. '
                        'Verificare categoria (OG/OS) e classifica nel disciplinare.'
                        if inferred_soa else
                        'Verificare categoria SOA (OG/OS) e classifica richieste nel disciplinare (lavori > €150K).')
        for doc in procurement.required_documents({'value_eur': float(value) if value else 0}):
            base_items.append((
                doc['name'],
                {'Qualifica': 'Bid manager', 'Garanzie': 'Commercial lead',
                 'Offerta': 'Estimator', 'Economico-finanziaria': 'Finance lead'}.get(doc['phase'], 'Bid manager'),
                (f"{doc['note']} Validità: {doc['validity']}." if doc['name'] != 'Attestazione SOA' else soa_evidence),
                'Critical' if doc['mandatory'] else 'High',
                'Not started',
                f"D.Lgs 36/2023 — {doc['phase']}",
            ))
        # CIG / ANAC tracking row
        base_items.insert(0, ('CIG / riferimento ANAC', 'Bid manager', cig_evidence,
                              'Critical', 'Not started', 'Art. 19 D.Lgs 36/2023'))
        if pnrr['is_pnrr']:
            base_items.append(('Obblighi PNRR (DNSH, quote giovani/donne, tracciabilità)', 'Compliance lead',
                               f"Segnali PNRR: {', '.join(pnrr['signals'][:4])}. Verificare clausole DNSH e premialità nel bando.",
                               'High', 'Not started', 'PNRR / NextGenerationEU'))
    ...
    base_items += [
        ...
        ('Eligibility and exclusion grounds', 'Commercial lead', 'Company registration, tax/social declarations, exclusion/self-declaration forms.', 'Critical', 'Not started', 'Art. 94-95 D.Lgs 36/2023' if is_italy else assumption),
        ...
        ('Submission portal and format check', 'Bid manager', 'Portal account, file naming, signature type (CAdES/PAdES), language, deadline, upload confirmation.', 'Critical', 'Not started', assumption),
    ...
    return {
        ...
        'sources': sources,
        # --- Phase 2 procurement intelligence (Italy) ---
        'cig': cig,
        'inferred_soa_class': inferred_soa,
        'pnrr': pnrr if is_italy else {'is_pnrr': False, 'signals': [], 'mission': None},
        'jurisdiction': 'IT/D.Lgs 36-2023' if is_italy else 'EU/TED',
    }
