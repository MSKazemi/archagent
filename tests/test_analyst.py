...
from archagent.intelligence import analyst
from archagent.intelligence import compliance as compliance_agent


# ─── Helpers ──────────────────────────────────────────────────────────────────

def _fake_azure_response(content: dict, tokens: int = 500) -> tuple[dict, int]:
    """Simulate a successful _call_azure response (Responses API format)."""
    resp = {
...
class TestExtractText(unittest.TestCase):
    ...
    @patch('archagent.intelligence.analyst.subprocess.run')
    def test_digital_pdf_text_extracted(self, mock_run):
        ...
        with patch('archagent.intelligence.analyst.tempfile.NamedTemporaryFile') as mock_tmp:
            mock_tmp.return_value.__enter__ = lambda s: s
            mock_tmp.return_value.__exit__ = MagicMock(return_value=False)
            mock_tmp.return_value.name = '/tmp/fake.pdf'
            with patch('archagent.intelligence.analyst.Path.unlink'):
    ...
    def test_150_page_cap_triggers_truncation_warning(self):
        ...
        with patch('archagent.intelligence.analyst._extract_text', return_value=(long_text, 150, True)), \
             patch('archagent.intelligence.analyst._is_scanned', return_value=False), \
             patch('archagent.intelligence.analyst._call_azure', return_value=_fake_azure_response(
...
class TestErrorHandling(unittest.TestCase):
    ...
    @patch('archagent.intelligence.analyst._extract_text', side_effect=RuntimeError('pdftotext not found'))
    ...
    @patch('archagent.intelligence.analyst._extract_text', return_value=('Some text content ' * 100, 10, False))
    @patch('archagent.intelligence.analyst._is_scanned', return_value=False)
    @patch('archagent.intelligence.analyst._call_azure', side_effect=RuntimeError('Azure API HTTP 429: rate limit'))
    ...
    @patch('archagent.intelligence.analyst._extract_text', return_value=('Some text content ' * 100, 10, False))
    @patch('archagent.intelligence.analyst._is_scanned', return_value=False)
    @patch('archagent.intelligence.analyst._extract_text')
    def test_missing_azure_key_returns_config_step(self, *_):
        with patch.dict(os.environ, {'AZURE_OPENAI_KEY': '', 'AZURE_OPENAI_ENDPOINT': ''}):
            ...
            with patch('archagent.intelligence.analyst._azure_endpoint', side_effect=EnvironmentError('AZURE_OPENAI_KEY required')):
                with patch('archagent.intelligence.analyst._extract_text', return_value=('text ' * 100, 10, False)), \
                     patch('archagent.intelligence.analyst._is_scanned', return_value=False):
                    result = analyst.analyze_tender(b'fake')
        self.assertEqual(result['status'], 'error')
...
class TestSuccessPath(unittest.TestCase):
    ...
    @patch('archagent.intelligence.analyst._extract_text', return_value=('Requisiti SOA OG11 Classifica IV\n' * 500, 80, False))
    @patch('archagent.intelligence.analyst._is_scanned', return_value=False)
    @patch('archagent.intelligence.analyst._call_azure')
    ...
    @patch('archagent.intelligence.analyst._extract_text', return_value=('text ' * 100, 80, False))
    @patch('archagent.intelligence.analyst._is_scanned', return_value=False)
    @patch('archagent.intelligence.analyst._call_azure')
...
class TestIntegration(unittest.TestCase):
    ...
    def test_real_azure_call_with_sample_text(self):
        ...
        with patch('archagent.intelligence.analyst._extract_text', return_value=(sample_text, 5, False)), \
             patch('archagent.intelligence.analyst._is_scanned', return_value=False):
